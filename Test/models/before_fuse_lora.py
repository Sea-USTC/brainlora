...
from models.projft_lora import LoRALinear

class beforeFuse(nn.Module):
    def __init__(self, config, number=4):
        super(beforeFuse, self).__init__()
        self.d_model = config['d_model']
        self.res_linear1=LoRALinear(config['r_proj'], config['lora_alpha'], self.d_model*number, self.d_model)
        self.res_linear2=LoRALinear(config['r_proj'], config['lora_alpha'], self.d_model, self.d_model)

...
def test(args,config):
    ...
    for i in range(4):
        model[i] = nn.DataParallel(model[i], device_ids = [i for i in range(torch.cuda.device_count())]) 
        # model = model.module
        model[i] = model[i].to(device)

    print('Load model from checkpoint:',args.model_path)
    checkpoint = torch.load(args.model_path,map_location='cpu') 
    for idx, model_para in enumerate(checkpoint['model']):
        model[idx].load_state_dict(model_para)
    ...
    fids = []
    pred_list = [pred for _ in range(len(model))]
    ...
    rootdir = "/remote-home/jianghaoyun/UniBrain-v2_new/Test/preds"+pred_name+main_name+"/"
        ...
        for i, sample in enumerate(test_dataloader):
            ...
            with torch.no_grad():
                ...
                for idx,cur_model in enumerate(model):
                    pred_class = cur_model(fuse_image_feature,cur_text_features, return_ws=False) #batch_size,num_class,1
                ...
                pred = torch.cat((pred, pred_mean.detach().cpu()), 0)
                # pred_list[idx] = torch.cat((pred_list[idx], pred_class.detach().cpu()), 0)
    ...
    xlsx_folder_path = '/remote-home/jianghaoyun/UniBrain-v2_new/Test/xlsx_files/'
    xlsx_file_path = xlsx_folder_path + 'ride_'+ pred_name+'.xlsx'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='/remote-home/jianghaoyun/UniBrain-v2_new/Test/configs/config_onefifth.yaml')
    parser.add_argument('--model_path', default='/remote-home/jianghaoyun/UniBrain-v2_new/Pretrain/output_ride_notemperature/best_val.pth')

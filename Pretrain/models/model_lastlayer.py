...
class MedKLIP_lastlayer(nn.Module):

    def __init__(self, config, mode='train'):
        super(MedKLIP_lastlayer, self).__init__()
        ...
        self.H = config['H'] 
        decoder_layer = TransformerDecoderLayer_lora(self.d_model, config['H'] , 1024,
                                        0.1, config['r'], config['lora_alpha'],'relu',normalize_before=True)
        self.decoder_norm = nn.LayerNorm(self.d_model)
        self.decoder = TransformerDecoder_lora(decoder_layer, 1 , self.decoder_norm,
    ...
    def forward(self, image_feature, text_features, weights, ana_features=None, sample_index=None, celoss=False):
        # image_features 4, b,2352,768
        B = image_feature.shape[1]
        ...
        feature,ws = self.decoder(text_features, image_feature, weights,

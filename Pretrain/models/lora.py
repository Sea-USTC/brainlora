...
class MultiLoRALinear(LoRALayer):
    ...
    def forward(self, x, weights):
        # x (2352,16,768) (patch_num,batch_size,channel) weights (16,3) (batch_size,lora_num)
        if not self.merged:
            result = F.linear(x, self.linear.weight, bias=self.linear.bias) # (2352,16,768)
            ...
            weights = weights.unsqueeze(0).unsqueeze(-1) # (1,16,3,1)

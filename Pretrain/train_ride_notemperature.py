...
def train(model, image_encoder, text_encoder, fuseModule, tokenizer, data_loader, optimizer, epoch, warmup_steps, device, scheduler, config):
    ...
    per_class_num = [647,125,1902,314,17,28,146,165,219,564,78,597,2361]
    RIDE_loss = RIDELoss(cls_num_list=None)
    # model.train()
    ...
    for i, sample in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        for cur_model in model:
            logits = cur_model(fuse_image_feature,cur_text_features,cur_ana_features)
        ...
        pred_mean = torch.mean(torch.stack(pred_class_list,dim=0),dim=0)
        ...
        #     logits = logits[Mask]
        #     logits = logits[:,0]
        logits_list_mask = [logits[Mask][:,0] for logits in logits_list]
        logits_mean = torch.mean(torch.stack(logits_list_mask,dim=0),dim=0)
        labels = labels[:,0]
        ...
        # per_class_num = [647,125,1902,314,17,28,146,165,219,564,78,597,2361]
        # per_class_temperature = per_class_num * B
        # # per_class_temperature = torch.tensor([element / sum(per_class_temperature) for element in per_class_temperature]).to(device)
        # per_class_temperature = per_class_temperature[Mask]
        loss_ce = RIDE_loss(logits_mean,labels,logits_list_mask,Mask,B)
        ...
        tensorboard["pred"] = torch.cat((tensorboard["pred"], pred_mean), 0)
...
def main(args, config):
    ...
    model = [MedKLIP(config) for _ in range(4)]
    ...
    for i in range(4):
        model[i] = nn.DataParallel(model[i], device_ids) 
        # model = model.module
        model[i] = model[i].cuda(device=device_ids[0])
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        if utils.is_main_process():  
            ...
            model_params = [cur.state_dict() for cur in model]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='/remote-home/jianghaoyun/UniBrain-v2_new/Pretrain/output_ride_notemperature')

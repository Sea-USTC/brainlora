...
def train(model, image_encoder, weight_MLP, project_MLP, text_encoder, fuseModule, tokenizer, data_loader, optimizer, epoch, warmup_steps, device, scheduler, config):
    ...
    metric_logger.add_meter('loss_w_ce', utils.SmoothedValue(window_size=50, fmt='{value:.6f}'))
    metric_logger.add_meter('loss_w_cl', utils.SmoothedValue(window_size=50, fmt='{value:.6f}'))
    metric_logger.update(loss=1.0)
    metric_logger.update(loss_ce=1.0)
    metric_logger.update(loss_cl=1.0)
    metric_logger.update(loss_clip=1.0)
    metric_logger.update(loss_w_ce=1.0)
    metric_logger.update(loss_w_cl=1.0)
    ...
    for i, sample in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        dis_labels = sample['dis_label'].to(device)
        ...
        weights_project = project_MLP(weights_norm)
        
        if config['w_bce'] == True:
            # logits = torch.sigmoid(weights/config['tau'])
            # loss_w_ce = F.binary_cross_entropy(logits,dis_labels.float())
            trans_matrix_w = torch.tensor([[1],[0.5],[0.25]]).float().to(device)
            trans_matrix_l = torch.tensor([[4/7],[2/7],[1/7]]).float().to(device)
            w_score = torch.matmul(weights_norm.float(),trans_matrix_w).squeeze()
            l_score = torch.matmul(dis_labels.float(),trans_matrix_l).squeeze()
            loss_w_ce = mse_loss(w_score,l_score)
            
        if config['w_cl'] == True:
            trans_matrix = torch.tensor([[4],[2],[1]]).float().to(device)
            label_dis = torch.matmul(dis_labels.float(),trans_matrix).squeeze()
            loss_w_cl = weight_cl_loss(weights_project,label_dis)

        if config['no_cl'] == False:
            logits, ll, cl_labels = model(fuse_image_feature,cur_text_features,weights_norm.detach(),cur_ana_features)
        else:
            logits = model(fuse_image_feature,cur_text_features,weights_norm.detach(),cur_ana_features)
        ...
        loss = loss_ce * loss_ce_ratio + loss_cl + loss_clip * config['kad_loss_ratio'] + loss_w_ce + loss_w_cl
        ...
        if config['w_bce']:
            tensorboard["train_loss_w_ce"].append(loss_w_ce.item())
        if config['w_cl']:
            tensorboard["train_loss_w_cl"].append(loss_w_cl.item())
        # if config['num_classes']>13:
        ...
        metric_logger.update(loss_w_ce=loss_w_ce.item())
        metric_logger.update(loss_w_cl=loss_w_cl.item())
...
def valid(model, image_encoder, weight_MLP, project_MLP, text_encoder, fuseModule, tokenizer, data_loader, device,config):
    mask_modal = config['mask_modal'] if 'mask_modal' in config else ""
    clip_loss = ClipLoss(mask_modal=mask_modal)
    mse_loss = nn.MSELoss()
    model.eval()
    ...
    for i, sample in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            if config['no_cl'] == False:
                logits, ll, cl_labels = model(fuse_image_feature,cur_text_features,weights_norm,cur_ana_features)
            else:
                logits = model(fuse_image_feature,cur_text_features,weights_norm,cur_ana_features)
            ...
            tensorboard["val_loss_w_ce"].append(loss_w_ce.item())
            tensorboard["val_loss_w_cl"].append(loss_w_cl.item())
            # if config['no_cl'] == False:
...
def main(args, config):
    ...
    weight_MLP = nn.DataParallel(weight_MLP, device_ids) 
    weight_MLP = weight_MLP.cuda(device=device_ids[0])
    
    project_MLP = nn.DataParallel(project_MLP, device_ids) 
    project_MLP = project_MLP.cuda(device=device_ids[0])
    ...
    optimizer = create_optimizer(arg_opt, model, image_encoder, text_encoder, fuseModule, weight_MLP, project_MLP)
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        train_stats, tensorboard_train = train(model, image_encoder, weight_MLP, project_MLP, text_encoder, fuseModule, tokenizer, train_dataloader, optimizer, epoch, warmup_steps, device, lr_scheduler, config) 

        writer.add_scalar('lr/leaning_rate',  lr_scheduler._get_lr(epoch)[0] , epoch)

        tensorboard_val = valid(model, image_encoder, weight_MLP, project_MLP, text_encoder, fuseModule, tokenizer, val_dataloader, device, config)
        ...
        if config['w_bce']:
            writer.add_scalars('loss_w_ce',{'train_loss':tensorboard_train["train_loss_w_ce"],"val_loss":tensorboard_val["val_loss_w_ce"]}, epoch)
        if config['w_cl']:
            writer.add_scalars('loss_w_cl',{'train_loss':tensorboard_train["train_loss_w_cl"],"val_loss":tensorboard_val["val_loss_w_cl"]}, epoch)
        ...
        if utils.is_main_process():  
            ...
            log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                         'epoch': epoch, 'val_loss_ce': tensorboard_val["val_loss_ce"],'val_loss_w_ce':tensorboard_val["val_loss_w_ce"],
                         'val_loss_w_cl':tensorboard_val["val_loss_w_cl"]
                        }                     
            save_obj = {
                'model': model.state_dict(),
                'image_encoder': image_encoder_params,
                'fuseModule': fuseModule.state_dict(),
                'weight_MLP':weight_MLP.state_dict(),
                'project_MLP':project_MLP.state_dict(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='/remote-home/jianghaoyun/UniBrain-v2_new/Pretrain/configs/config_test.yaml')
    parser.add_argument('--finetune_checkpoint', default='/remote-home/jianghaoyun/UniBrain-v2_new/Pretrain/output_dir/output_half_baseline/best_val.pth')
    parser.add_argument('--output_dir', default='/remote-home/jianghaoyun/UniBrain-v2_new/Pretrain/output_dir/output_test1')
